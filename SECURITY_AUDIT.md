...
- `/api/discovery/photo` validates signed photo URLs first, then applies a separate 100 requests per day per client IP limit when Upstash is configured.
- `/api/discovery/photo` also enforces a global 940-request photo budget per UTC calendar month.
