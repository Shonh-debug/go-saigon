...
3. **Set up your environment variables**
   ...
   > Upstash Redis can be left blank for local development, but it is required in production. Without Upstash configured on Vercel, discovery and photo routes fail closed with `503` instead of allowing unlimited public Google API traffic.
...
| `UPSTASH_REDIS_REST_URL` | Upstash Redis REST URL used to rate-limit public discovery and photo routes |
| `UPSTASH_REDIS_REST_TOKEN` | Upstash Redis REST token used to rate-limit public discovery and photo routes |
