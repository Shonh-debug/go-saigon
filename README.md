...
The current version is built as a polished local demo for a future Vercel deployment. It uses a growing dataset of recent locations Ho Chi Minh City as the baseline and can manually enrich those places with Google Places API data through a server-side sync endpoint.
